...
import './App.css'
import { words, length } from './assets/words'
...
function App() {
  ...
  const generateNewWord = () => {
    wordSync = words[Math.floor(Math.random() * length)].toUpperCase();
    setWord(wordSync + "");
    setActiveIndex(-1);
    colorsSync = [[]]
  ...
  const onEnter = (input:string[]) => {
    let wordInput = "";
    input.forEach((char:string) => {wordInput += char.toLowerCase()});

    let color : Colors[] = []; 
    let tempKeyboardColors = keyboardColors;

    if (!words.includes(wordInput)) return false;
    ...
    if (color.every((c :Colors) => {return c == Colors.Green})) {
      setShowButton(true);
  ...
  return (
    <>
      ...
      {Array.from({length: activeIndex + 1}).map((_,index:number) => (
        <SpacesComponent ref={spacesRef}key={index} active={index == activeIndex} onEnter={onEnter} color={colors[index]}/>
      ))}
      <KeyboardComponent greenKeys={keyboardColors[0]} yellowKeys={keyboardColors[1]} greyKeys={keyboardColors[2]} onClick={keyPress}/>
      {showButton && (
        <ResultsComponent word={word} generateNewWord={generateNewWord} show={showButton} scores={scores} streak={streak}/>
      )}
    </>
  )
}
